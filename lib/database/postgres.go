...
func newPostgresDatabase(host, port, user, password string) (db *PostgresDatabase, err error) {
...
// ensure tables
func (self *PostgresDatabase) Ensure() (err error) {
	log.WithFields(log.Fields{
		"pkg": "db-pq",
	}).Info("Ensure Database")

	for err == nil {
		version := self.getDBVersion()
		if version == -1 {
			// no tables
			err = self.createTablesV0()
		} else if version == 0 {
			// upgrade to version 1
			err = self.upgrade0to1()
		} else if version == 1 {
			// upgrade to version 2
			err = self.upgrade1to2()
		} else if version == 2 {
			// upgrade to version 3
			err = self.upgrade2to3()
		} else if version == 3 {
			// update to version 4
			err = self.upgrade3to4()
		} else if version == 4 {
			// update to version 5
			err = self.upgrade4to5()
		} else if version == 5 {
			// update to version 6
			break
			//err = self.upgrade5to6()
		} else if version == 6 {
			break
		}
		if err != nil {
...
func (self *PostgresDatabase) createTablesV0() (err error) {
	log.WithFields(log.Fields{
		"pkg": "db-pq",
	}).Debug("create version 0 tables")
	...
	for _, table := range table_order {
		...
		q := fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s%s", table, tables[table])
		log.WithFields(log.Fields{
			"pkg": "db-pq",
		}).Debug(q)
		// create table
		_, err = self.conn.Exec(q)
		if err != nil {
			log.WithFields(log.Fields{
				"pkg": "db-pq",
				"table", table,
	...
	// create indexes
	for _, t := range []string {"ArticleThreads(root_message_id)", "ArticleAttachments(message_id)", "ArticlePosts(message_id)", "Articles(message_id)", "Newsgroups(name)"} {
		q := "CREATE INDEX IF NOT EXISTS ON " + t
...
func (self *PostgresDatabase) upgrade0to1() (err error){
	...
	err = self.setDBVersion(1)
	return
}


func (self *PostgresDatabase) upgrade1to2() (err error) {
	...
	if err == nil {
		err = self.setDBVersion(2)
	} else {
		log.WithFields(log.Fields{
			"pkg": "db-pq",
		}).Error("error migrating 1 -> 2", err)
	}
	return
}

func (self *PostgresDatabase) upgrade2to3() (err error) {
	log.WithFields(log.Fields{
		"pkg": "db-pq",
		"table": table,
	}).Debug("migrate 2 -> 3")
...
func (self *PostgresDatabase) upgrade3to4() {
	...
	if err != nil {
		log.WithFields(log.Fields{
			"pkg": "db-pq",
		}).Error("could not migrate 3 -> 4", err)
		return
	}
	counter := int64(0)
	for rows.Next() {
		...
		if err != nil {
			log.WithFields(log.Fields{
				"pkg": "db-pq",
			}).Error("failed 3 -> 4 migration", err)
			return
		}
		...
		if err != nil {
			log.WithFields(log.Fields{
				"pkg": "db-pq",
			}).Error("failed 3 -> 4 migration, could not register nntp number", err)
			return
		}
		if counter%100 == 0 {
	...
	err = self.setDBVersion(4)
	return
}


func (self *PostgresDatabase) upgrade4to5() (err error) {
	log.WithFields(log.Fields{
		"pkg": "db-pq",
	}).Debug("migrate 4 -> 5")
	...
	err = self.setDBVersion(5)
	return
}

func (self *PostgresDatabase) upgrade5to6() (err error) {
	return
}
...
func (self *PostgresDatabase) getDBVersion() (version int) {
	...
	if err == nil {
		...
		if err == nil {
			...
			log.WithFields(log.Fields{
				"pkg": "db-pq",
			}).Fatal("failed to determine database version", err)
		}
	} else {
		version = -1
	}
...
func (self PostgresDatabase) GetPostsInGroup(newsgroup string) (models []model.Post, err error) {
